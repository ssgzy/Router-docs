...
- **type**: Channel identifier, used for backend routing and order placement.
  ...
  - For detailed logic, see the backend controller [controller/topup.go](https://github.com/ssgzy/Router-new-api/blob/main/controller/topup.go).
...
- For detailed implementation logic, see the backend controller [controller/topup.go](https://github.com/ssgzy/Router-new-api/blob/main/controller/topup.go).
...
Ratio settings are the core configuration of the Code Router billing system. By setting different Ratios, you can flexibly control the billing standards for various models and user groups.

### Ratio System Overview

Code Router uses a three-tier Ratio system to calculate user Quota consumption:
...
In the Code Router system, Ratios are key parameters for calculating Quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into Quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by Code Router)
...
Code Router employs a dual billing mechanism of pre-consumption and post-consumption:

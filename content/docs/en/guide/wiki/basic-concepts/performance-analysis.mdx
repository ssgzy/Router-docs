...
Code Router provides two types of performance analysis capabilities:
...
environment:
  ...
  - PYROSCOPE_APP_NAME=code-router
...
export PYROSCOPE_APP_NAME=code-router
...
```bash
# Check env vars
docker exec code-router env | grep -E "PPROF|PYROSCOPE"
```

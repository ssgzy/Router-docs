...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with Code Router.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with Code Router.
</Callout>
...
## Integrating Code Router

LangBot supports integrating with locally deployed Code Router and third-party Code Router services built using Code Router.

### Usage

1.  Obtain an API key from Code Router
    ![获取 API key](/assets/langbot/get_api_key.png)

        If Code Router is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party Code Router service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select Code Router as the provider, and fill in the corresponding API key and API address
...
LangBot supports using Code Router's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select Code Router as the provider

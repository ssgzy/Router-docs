...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with AI Router.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with AI Router.
</Callout>
...
## Integrating AI Router

LangBot supports integrating with locally deployed AI Router and third-party AI Router services built using AI Router.

### Usage

1.  Obtain an API key from AI Router
    ![获取 API key](/assets/langbot/get_api_key.png)

        If AI Router is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party AI Router service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select AI Router as the provider, and fill in the corresponding API key and API address
...
LangBot supports using AI Router's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select AI Router as the provider

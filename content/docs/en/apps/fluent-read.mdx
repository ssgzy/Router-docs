...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to Code Router for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the Code Router console, you can add
...
### Import Configuration from Code Router Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the Code Router console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the Code Router configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | Code Router                                  |
| Access Token        | Code Router Key                              |
| Code Router Endpoint     | Code Router Deployment Address (without /v1) |

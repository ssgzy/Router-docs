...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with AI Router to enable multi-model workflows.
---
...
Provider Type: Types supported by AI Router
API Key: Obtained from AI Router
API Address: AI Router site address (e.g., `https://ai.sammier.com/v1`)

Please use your self-deployed AI Router, or ensure that the service provider offers AI Router services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in AI Router**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select AI Router
    ![Add AI Router Provider](/assets/aionui/newapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your AI Router site address (format: `https://ai.sammier.com/v1`)
    - API Key: Paste the API Key copied from the AI Router console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in AI Router
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured AI Router model to start a conversation

...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with Code Router to enable multi-model workflows.
---
...
Provider Type: Types supported by Code Router
API Key: Obtained from Code Router
API Address: Code Router site address (e.g., `https://api.code-router.example/v1`)

Please use your self-deployed Code Router, or ensure that the service provider offers Code Router services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in Code Router**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select Code Router
    ![Add Code Router Provider](/assets/aionui/newapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your Code Router site address (format: `https://api.code-router.example/v1`)
    - API Key: Paste the API Key copied from the Code Router console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in Code Router
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured Code Router model to start a conversation

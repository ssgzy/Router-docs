...
- **Base URL** — `https://api.code-router.example` (the OpenAI-compatible endpoint is `https://api.code-router.example/v1`; Anthropic-compatible clients use the same base URL).
...
<Step>
**Set the Base URL.** Enter `https://api.code-router.example` (use `https://api.code-router.example/v1` if the field expects the OpenAI-compatible `/v1` path).
</Step>
...
```shell
export OPENAI_BASE_URL="https://api.code-router.example/v1"

...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with AI Router to improve translation quality.
---
...
## Integrating AI Router into LunaTranslator

LunaTranslator supports integration with locally deployed AI Router. Please use your own deployed AI Router, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their AI Router service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the AI Router console's `System Settings` -> `Chat Settings`, add the following shortcut option:
...
2.  In the **`AI Router`** -> `Console` -> `Token Management` tab, select the token you want to use in LunaTranslator, click the dropdown option next to the chat button, choose `LunaTranslator`, and you will be redirected to LunaTranslator with the API address and API Key automatically configured.
...
4.  Click the refresh button next to the **model** dropdown to retrieve the AI Router platform's model list, select or enter the model name, then click OK to save.
...
1.  In the **`AI Router`** -> `Console` -> `Token Management` tab, obtain the API Key.
...
5.  Click the refresh button next to the **model** dropdown to retrieve the AI Router platform's model list, select or enter the model name, then click OK to save.

    ![设置 API3](/assets/luna_translator/setting_api3.png)

6.  Click the toggle switch next to **AI Router** to enable the interface and start using it.

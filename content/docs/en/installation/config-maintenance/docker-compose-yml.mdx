...
This document details the Docker Compose configuration options for Code Router, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the Code Router service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the Code Router service itself:
...
### Code Router Service Configuration
...
| `environment`    | Environment variable settings, used to configure Code Router behavior              |
...
Code Router supports various environment variable configurations. Here are some commonly used ones:

...
description: A complete reference of the environment variables supported by Code Router, with defaults and examples for tuning the gateway's runtime behavior.
---

This document lists the environment variables supported by Code Router and how to configure them. You can customize the gateway's behavior by setting these variables before launching the service.

<Callout type="info" title="Tip">
  Code Router can read environment variables from a `.env` file. Copy the provided `.env.example` to `.env` and edit the values you need.
</Callout>
...
| Environment Variable          | Description                                    | Default Value             | Example                                                                                                                             |
| ----------------------------- | ---------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`                     | Database connection string                     | SQLite (`code-router.db`) | MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/code_router` \| PostgreSQL: `SQL_DSN=postgresql://root:123456@postgres:5432/code_router` |
...
| `LOG_SQL_DSN`                 | Separate database connection string for the log table | -                 | `LOG_SQL_DSN=root:123456@tcp(localhost:3306)/code_router_logs`                                                                  |
| `SQLITE_PATH`                 | SQLite database path                           | `code-router.db`          | `SQLITE_PATH=/var/lib/code-router/code-router.db`                                                                               |
...
| `FRONTEND_BASE_URL`               | Frontend base URL (used by slave nodes; unmatched routes redirect here) | -             | `FRONTEND_BASE_URL=https://api.code-router.example` |
...
<Callout type="warn" title="SESSION_SECRET Notes">
  `SESSION_SECRET` cannot be set to `random_string`, otherwise the program refuses to start. All nodes must share the same secret for multi-machine deployment.
</Callout>

<Callout type="info" title="Cluster Deployment">
  For how to build a complete cluster deployment using these environment variables, see the [Cluster Deployment Guide](/en/docs/installation/deployment-methods/cluster-deployment).
</Callout>
...
<Callout type="warn" title="RELAY_TIMEOUT Setting Warning">
  Be cautious when setting `RELAY_TIMEOUT`. Setting it too short can cause the following issues:

  - The upstream API has completed the request and charged, but local billing fails due to the timeout.
  - This leads to billing desynchronization, which may result in losses.
...
| Environment Variable                                              | Description                                    | Default Value | Example |
| ----------------------------------------------------------------- | ---------------------------------------------- | ------------- | ---- |
| `CHANNEL_UPSTREAM_MODEL_UPDATE_TASK_ENABLED`                      | Enable regular synchronization of upstream provider model lists | `true`        | `CHANNEL_UPSTREAM_MODEL_UPDATE_TASK_ENABLED=false` |
...
| Environment Variable             | Description                                                  | Default Value | Example                                 |
| -------------------------------- | ------------------------------------------------------------ | ------------- | ------------------------------------ |
| `FORCE_STREAM_OPTION`            | Override the client `stream_options` parameter, forcing retrieval of streaming usage information | `true`        | `FORCE_STREAM_OPTION=false`          |
...
| Environment Variable           | Description                                                  | Default Value | Example                               |
| ------------------------------ | ------------------------------------------------------------ | ------------- | ---------------------------------- |
| `UPDATE_TASK`                  | Whether to enable asynchronous task polling updates (image/video generation, etc.) | `true`        | `UPDATE_TASK=false`                |
| `TASK_QUERY_LIMIT`             | Maximum number of tasks queried per poll                     | `1000`        | `TASK_QUERY_LIMIT=500`             |
| `TASK_TIMEOUT_MINUTES`         | Asynchronous task timeout (minutes); after timeout the task is marked failed and refunded, 0 to disable | `1440`        | `TASK_TIMEOUT_MINUTES=720`         |
| `TASK_PRICE_PATCH`             | Task price patch: the listed models are billed per-use only, model names separated by commas | -             | `TASK_PRICE_PATCH=sora-2,sora-2-pro` |
...
| Environment Variable          | Description                                                  | Default Value | Example                                    |
| ----------------------------- | ------------------------------------------------------------ | ------------- | ------------------------------------------ |
| `VITE_REACT_APP_SERVER_URL`   | Frontend base URL for backend requests (effective at compile time) | -             | `VITE_REACT_APP_SERVER_URL=https://api.code-router.example` |

<Callout type="info" title="Compile-time Variables">
  `VITE_REACT_APP_SERVER_URL` is a frontend variable injected by Vite at compile time. It only takes effect during the frontend build and cannot be modified at runtime.
</Callout>
...
| `PYROSCOPE_APP_NAME`          | Application name for reporting                 | `code-router` | `PYROSCOPE_APP_NAME=my-gateway` |
...
| `HOSTNAME`                    | Hostname label for reporting                   | `code-router` | `HOSTNAME=node-1`            |
...
The following environment variables are deprecated. Use the corresponding options in the system settings interface instead:

| Environment Variable          | Alternative Method                                |
| ----------------------------- | ------------------------------------------------- |
| `GEMINI_MODEL_MAP`            | Set in System Settings → Model Related Settings |
| `GEMINI_SAFETY_SETTING`       | Set in System Settings → Model Related Settings |
| `GEMINI_VISION_MAX_IMAGE_NUM` | Set in System Settings → Model Related Settings |
...
SQL_DSN=root:password@tcp(db-server:3306)/code_router
...
<Callout type="info" title="Complete Cluster Configuration">
  This is just a basic multi-node configuration example. For the full cluster deployment configuration, architecture description, and best practices, see the [Cluster Deployment Guide](/en/docs/installation/deployment-methods/cluster-deployment).
</Callout>

## Environment Variables in Docker Compose

Below is a brief example of setting environment variables in a Docker Compose configuration file. Replace the image reference with the one you build or pull for your deployment (image coordinates TBD — see the [source repository](https://github.com/ssgzy/Router-new-api)).

```yaml
services:
  code-router:
    image: <your-code-router-image>:latest
    environment:
      - TZ=UTC
      - SQL_DSN=root:123456@tcp(mysql:3306)/code_router
...
For a complete Docker Compose configuration, including more environment variable options, see the [Docker Compose Configuration Instructions](/en/docs/installation/config-maintenance/docker-compose-yml).

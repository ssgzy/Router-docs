...
description: OpenAI Codex CLI 安装与配置教程 — 在 Windows、macOS、Linux 上安装终端 AI 编程助手，通过 OpenAI 兼容接口对接 AI Router。
---
...
<Cards>
  <Card title="Base URL（接入地址）" description="OpenAI 兼容接口地址，形如 https://ai.sammier.com/v1。请以你自己部署或使用的 AI Router 实例地址为准。" />
  <Card title="API Key（密钥）" description="形如 sk-... 的 AI Router 密钥。妥善保管，不要提交到代码仓库或公开分享。" />
</Cards>
...
<Callout type="info" title="字段说明">
  - `base_url`：填 AI Router 的 OpenAI 兼容地址（`https://ai.sammier.com/v1`）。请替换为你自己部署或使用的实例地址。
  - `env_key`：指定读取密钥的环境变量名，对应上一步设置的 `OPENAI_API_KEY`。
  - `model`：填你在 AI Router 中可用的模型名（具体可用模型与分组以你的 AI Router 控制台为准）。
</Callout>
...
- 输入 `/model` 切换模型（需是 AI Router 中可用的模型名）。
...
<Accordions>
  <Accordion title="提示鉴权失败 / 401 Unauthorized">
    - 确认 `OPENAI_API_KEY` 已正确设置为你的 AI Router 密钥（`echo $OPENAI_API_KEY` 检查）。
    - 确认 `config.toml` 中 `env_key` 与实际环境变量名一致。
    - 修改环境变量后需重新打开终端。
  </Accordion>
  <Accordion title="提示模型不存在 / 404">
    检查 `config.toml` 中 `model` 填写的名称是否为 AI Router 中实际开通的模型名（不同实例的可用模型与分组不同，请以控制台为准）。
  </Accordion>
  <Accordion title="连接超时 / 无法访问接入点">
    - 确认 `base_url` 正确，且结尾为 `/v1`。
    - 确认网络可以访问你的 AI Router 实例地址。
  </Accordion>
...
使用 AI Router 过程中遇到问题，可在源码仓库提交 issue：[https://github.com/ssgzy/Router-new-api](https://github.com/ssgzy/Router-new-api)

...
本文档提供了在本地环境中设置和开发 Code Router 项目的详细步骤，适合希望参与项目开发或进行功能定制的开发者。
...
首先，从 GitHub 克隆 Code Router 仓库到本地：
...
Code Router 支持通过 `.env` 文件配置环境变量。创建 `.env` 文件（可从 `.env.example` 复制）：
...
Code Router 的前端代码位于 `web` 目录中，使用 React 和 [semi design 组件库](https://semi.design/zh-CN) 开发。

...
本文档提供使用 1Panel 面板部署 Code Router 的图文教程。
...
<Callout type="info" title="搜索应用">
  进入应用商店应用列表，在【AI】分类下找到 Code Router 应用进行安装
</Callout>
...
<Callout type="info" title="安装应用">
  在安装页面配置 Code Router 应用参数：
</Callout>

- **名称**：要创建的 Code Router 应用的名称
- **版本**：选择 Code Router 的版本
- **端口**：Code Router 应用的服务端口
...
<Callout type="info" title="确认安装">
  点击确认开始安装，页面将自动跳转到已安装应用列表，等待 Code Router 应用状态变为已启动
</Callout>
...
1. 在 1Panel 应用列表中，点击 Code Router 应用的 **参数** 按钮
2. 在环境变量区域添加：
...
<Callout type="info" title="访问应用">
  - 在 `已安装` 页面，点击 `跳转` 即可进入 Code Router 的 WebUI 页面
...
- [GitHub 仓库](https://github.com/ssgzy/Router-new-api)